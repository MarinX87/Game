...
void InitLight(void)
{
	...
	g_Light[1].Diffuse = XMFLOAT4(0.6f, 0.6f, 0.6f, 1.0f);	// 光の色
	g_Light[1].Type = LIGHT_TYPE_DIRECTIONAL;					// 並行光源
	g_Light[1].Enable = TRUE;									// このライトをON
	SetLight(1, &g_Light[1]);									// これで設定している

		// 並行光源の設定（世界を照らす光）
	g_Light[2].Direction = XMFLOAT3(0.0f, -1.0f, -1.0f);		// 光の向き
	g_Light[2].Diffuse = XMFLOAT4(0.6f, 0.6f, 0.6f, 1.0f);	// 光の色
	g_Light[2].Type = LIGHT_TYPE_DIRECTIONAL;					// 並行光源
	g_Light[2].Enable = TRUE;									// このライトをON
	SetLight(2, &g_Light[2]);									// これで設定している
	// フォグの初期化（霧の効果）
	g_Fog.FogStart = 380.0f;									// 視点からこの距離離れるとフォグがかかり始める
	g_Fog.FogEnd   = 500.0f;									// ここまで離れるとフォグの色で見えなくなる
